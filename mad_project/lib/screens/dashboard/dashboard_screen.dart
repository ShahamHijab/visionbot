...
class _HomeTabState extends State<HomeTab> with SingleTickerProviderStateMixin {
  ...
  _AlertUI _mapAlertUI(AlertModel alert) {
    final title = _titleFromType(alert.type.toString().split('.').last);
    ...
    final color = _colorFromType(alert.type.toString().split('.').last);
    final icon = _iconFromType(alert.type.toString().split('.').last);
    ...
    try {
      ...
      if (user != null) {
        ...
        setState(() {
          ...
          _userRole = role == 'admin' ? 'Administrator' : 
                      role == 'officer' ? 'Security Officer' : 'User';
          _loading = false;
        });
      } else {
        setState(() {
